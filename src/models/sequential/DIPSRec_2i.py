...
class DIPSRec_2i(SequentialModel, DIPSRec_2iBase):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = DIPSRec_2iBase.parse_model_args(parser)
    ...
    def forward(self, feed_dict):
        out_dict = DIPSRec_2iBase.forward(self, feed_dict)
        # return {'prediction': out_dict['prediction']}
        return {'prediction': out_dict['prediction'], 'kl': out_dict['kl']}
    
class DIPSRec_2iImpression(ImpressionSeqModel, DIPSRec_2iBase):
    ...
    def forward(self, feed_dict):
        return DIPSRec_2iBase.forward(self, feed_dict)
...
class RtMIPSRec_frequency512Base(object):
    @staticmethod
    def parse_model_args(parser):
        ...
        parser.add_argument('--time_size', type=int, default=512,
                            help='Size of embedding vectors.')
    ...
    def forward(self, feed_dict):
        ...
        # 算法输入部分
        u_ids = feed_dict['user_id']  # 用户id，RtMIPSRec_frequency512不对用户特征直接建模，因此不使用用户id信息
        ...
        attn_mask_full = torch.ones_like(attn_mask) # RtMIPSRec_frequency512不同于SASRec架构，其不使用顺序掩码
...
class RtMIPSRec_frequency512(SequentialModel, RtMIPSRec_frequency512Base):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = RtMIPSRec_frequency512Base.parse_model_args(parser)
    ...
    def forward(self, feed_dict):
        out_dict = RtMIPSRec_frequency512Base.forward(self, feed_dict)

...
class MIPSRec_hw06(SequentialModel, MIPSRec_hw06Base):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = MIPSRec_hw06Base.parse_model_args(parser)
    ...
    def get_gram_matrix(self, dataset):
        ...
        gram_matrix = gram_matrix * 0.6 + gram_matrix_r2 * 0.4
    ...
    def forward(self, feed_dict):
        out_dict = MIPSRec_hw06Base.forward(self, feed_dict)
        # return {'prediction': out_dict['prediction']}
        return {'prediction': out_dict['prediction'], 'kl': out_dict['kl']}
    
class MIPSRec_hw06Impression(ImpressionSeqModel, MIPSRec_hw06Base):
    ...
    def forward(self, feed_dict):
        return MIPSRec_hw06Base.forward(self, feed_dict)
...
class DIPSRec_TIP(SequentialModel, DIPSRec_TIPBase):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = DIPSRec_TIPBase.parse_model_args(parser)
    ...
    def forward(self, feed_dict):
        out_dict = DIPSRec_TIPBase.forward(self, feed_dict)
        # return {'prediction': out_dict['prediction']}
        return {'prediction': out_dict['prediction'], 'kl': out_dict['kl']}
    
class DIPSRec_TIPImpression(ImpressionSeqModel, DIPSRec_TIPBase):
    ...
    def forward(self, feed_dict):
        return DIPSRec_TIPBase.forward(self, feed_dict)
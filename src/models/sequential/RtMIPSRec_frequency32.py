...
class RtMIPSRec_frequency32Base(object):
    @staticmethod
    def parse_model_args(parser):
        ...
        parser.add_argument('--time_size', type=int, default=32,
                            help='Size of embedding vectors.')
    ...
    def forward(self, feed_dict):
        ...
        # 算法输入部分
        u_ids = feed_dict['user_id']  # 用户id，RtMIPSRec_frequency32不对用户特征直接建模，因此不使用用户id信息
        ...
        attn_mask_full = torch.ones_like(attn_mask) # RtMIPSRec_frequency32不同于SASRec架构，其不使用顺序掩码
...
class RtMIPSRec_frequency32(SequentialModel, RtMIPSRec_frequency32Base):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = RtMIPSRec_frequency32Base.parse_model_args(parser)
    ...
    def forward(self, feed_dict):
        out_dict = RtMIPSRec_frequency32Base.forward(self, feed_dict)

...
class TiDIPSRec_0d1Base(object):
    @staticmethod
    ...
    def forward(self, feed_dict):
        ...
        time_attenuation = period[None, None, :] / (period[None, None, :] + 0.1 * current_interval[:, :, None])
...
class TiDIPSRec_0d1(SequentialModel, TiDIPSRec_0d1Base):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = TiDIPSRec_0d1Base.parse_model_args(parser)
    ...
    def forward(self, feed_dict):
        out_dict = TiDIPSRec_0d1Base.forward(self, feed_dict)
        # return {'prediction': out_dict['prediction']}
        return {'prediction': out_dict['prediction'], 'kl': out_dict['kl']}
    
class TiDIPSRec_0d1Impression(ImpressionSeqModel, TiDIPSRec_0d1Base):
    ...
    def forward(self, feed_dict):
        return TiDIPSRec_0d1Base.forward(self, feed_dict)
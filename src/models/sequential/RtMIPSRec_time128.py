...
class RtMIPSRec_time128Base(object):
    @staticmethod
    def parse_model_args(parser):
        ...
        parser.add_argument('--time_size', type=int, default=256,
                            help='Size of embedding vectors.')
        parser.add_argument('--max_time', type=int, default=128,
    ...
    def forward(self, feed_dict):
        ...
        # 算法输入部分
        u_ids = feed_dict['user_id']  # 用户id，RtMIPSRec_time128不对用户特征直接建模，因此不使用用户id信息
        ...
        attn_mask_full = torch.ones_like(attn_mask) # RtMIPSRec_time128不同于SASRec架构，其不使用顺序掩码
...
class RtMIPSRec_time128(SequentialModel, RtMIPSRec_time128Base):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = RtMIPSRec_time128Base.parse_model_args(parser)
    ...
    def forward(self, feed_dict):
        out_dict = RtMIPSRec_time128Base.forward(self, feed_dict)

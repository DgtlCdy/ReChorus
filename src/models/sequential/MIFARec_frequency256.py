...
class MIFARec_frequency256Base(object):
    @staticmethod
    ...
    def forward(self, feed_dict):
        ...
        # 算法输入部分
        u_ids = feed_dict['user_id']  # 用户id，MIFARec_frequency256不对用户特征直接建模，因此不使用用户id信息
        ...
        attn_mask_full = torch.ones_like(attn_mask) # MIFARec_frequency256不同于SASRec架构，其不使用顺序掩码
...
class MIFARec_frequency256(SequentialModel, MIFARec_frequency256Base):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = MIFARec_frequency256Base.parse_model_args(parser)
    ...
    def forward(self, feed_dict):
        out_dict = MIFARec_frequency256Base.forward(self, feed_dict)

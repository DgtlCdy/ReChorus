...
class MIFARec_frq1_sqBase(object):
    @staticmethod
    ...
    def forward(self, feed_dict):
        ...
        # 算法输入部分
        u_ids = feed_dict['user_id']  # 用户id，MIFARec_frq1_sq不对用户特征直接建模，因此不使用用户id信息
        ...
        attn_mask_full = torch.ones_like(attn_mask) # MIFARec_frq1_sq不同于SASRec架构，其不使用顺序掩码
...
class MIFARec_frq1_sq(SequentialModel, MIFARec_frq1_sqBase):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = MIFARec_frq1_sqBase.parse_model_args(parser)
    ...
    def forward(self, feed_dict):
        out_dict = MIFARec_frq1_sqBase.forward(self, feed_dict)

...
class MIPSRec_top2000Base(object):
    @staticmethod
...
class MIPSRec_top2000(SequentialModel, MIPSRec_top2000Base):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = MIPSRec_top2000Base.parse_model_args(parser)
    ...
    def get_gram_matrix(self, dataset):
        ...
        # 方法2：取top相似度
        # 取top500的相似度去做
        indices = torch.topk(gram_matrix, 2000, dim=1).indices
    ...
    def forward(self, feed_dict):
        out_dict = MIPSRec_top2000Base.forward(self, feed_dict)
        # return {'prediction': out_dict['prediction']}
        return {'prediction': out_dict['prediction'], 'kl': out_dict['kl']}
    
class MIPSRec_top2000Impression(ImpressionSeqModel, MIPSRec_top2000Base):
    ...
    def forward(self, feed_dict):
        return MIPSRec_top2000Base.forward(self, feed_dict)
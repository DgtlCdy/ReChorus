...
class MIPSRec_hw07(SequentialModel, MIPSRec_hw07Base):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = MIPSRec_hw07Base.parse_model_args(parser)
    ...
    def get_gram_matrix(self, dataset):
        ...
        gram_matrix = gram_matrix * 0.7 + gram_matrix_r2 * 0.3
    ...
    def forward(self, feed_dict):
        out_dict = MIPSRec_hw07Base.forward(self, feed_dict)
        # return {'prediction': out_dict['prediction']}
        return {'prediction': out_dict['prediction'], 'kl': out_dict['kl']}
    
class MIPSRec_hw07Impression(ImpressionSeqModel, MIPSRec_hw07Base):
    ...
    def forward(self, feed_dict):
        return MIPSRec_hw07Base.forward(self, feed_dict)
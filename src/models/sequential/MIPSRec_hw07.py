...
class MISRec_hw07(SequentialModel, MISRec_hw07Base):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = MISRec_hw07Base.parse_model_args(parser)
    ...
    def forward(self, feed_dict):
        out_dict = MISRec_hw07Base.forward(self, feed_dict)
        # return {'prediction': out_dict['prediction']}
        return {'prediction': out_dict['prediction'], 'kl': out_dict['kl']}
    
class MISRec_hw07Impression(ImpressionSeqModel, MISRec_hw07Base):
    ...
    def forward(self, feed_dict):
        return MISRec_hw07Base.forward(self, feed_dict)
...
class MIFARec_ebd2_lineBase(object):
    @staticmethod
    ...
    def forward(self, feed_dict):
        ...
        # 算法输入部分
        u_ids = feed_dict['user_id']  # 用户id，MIFARec_ebd2_line不对用户特征直接建模，因此不使用用户id信息
        ...
        attn_mask_full = torch.ones_like(attn_mask) # MIFARec_ebd2_line不同于SASRec架构，其不使用顺序掩码
...
class MIFARec_ebd2_line(SequentialModel, MIFARec_ebd2_lineBase):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    @staticmethod
    def parse_model_args(parser):
        parser = MIFARec_ebd2_lineBase.parse_model_args(parser)
    ...
    def forward(self, feed_dict):
        out_dict = MIFARec_ebd2_lineBase.forward(self, feed_dict)
